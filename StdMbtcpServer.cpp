...
void show_prog_info()
{
	...
	cout << "Program name StdMbtcpServer run with hardware v1." << endl;
...
int  process_socket_data(int epfd, int sockfd, StdModbusTcp & std_modbus_tcp)
{
	...
	while(1){
		...
		for(i=0;i<nfds;++i){
			if(events[i].data.fd == listenfd){
				...
				if(process_socket_data(epfd, events[i].data.fd, std_modbus_tcp)<0){
					events[i].data.fd = -1;
				}
			}
		}
